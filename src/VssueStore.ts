...
class VssueStore {
  ...
  setOptions(options: Partial<Vssue.Options>): void {
    ...
    if (this.options.locale) {
      setI18nLocale(this.options.locale as LOCALE_ENUM)
    } else {
      const locales = Object.keys(i18n.global.messages);
      const navLangs = window.navigator.languages;
      setI18nLocale((navLangs.filter(item => locales.includes(item)).shift() || getDefaultLocale()) as LOCALE_ENUM)        
    }
  }
